...
2. **バックアップの有無を確認する**
   - 🔴 **本番 Supabase は Free プランであり、Supabase 側の自動バックアップ・PITR は存在しない。**
     Dashboard の Settings → Database → Backups を見ても復旧の材料は無い。**探しに行かないこと。**
   - 復旧の材料になるのは、日次スナップショット（[backup-design.md](./backup-design.md)）のみ。
     管理画面の `backup_runs` で最終成功時刻を確認する。
   - ⚠ **2026-09-16時点：日次スナップショットは設計確定・実装未着手。**
     この時点で物理削除が起きた場合、**戻す手段は無い**（下記「復旧手順」2 を参照）。
3. **影響範囲調査**
   ```sql
...
1. **論理削除（is_deleted）の場合：** UPDATE で戻せる
   ...
   where /* 条件 */;
   ```
2. **物理削除・データ破損の場合：** 日次スナップショットから戻す
   - 🔴 **Point-in-Time Restore（任意時点への復元）は使えない。** PITR は Pro 以上のアドオン（月額約$100〜）で、
     本番は Free プラン。**戻せるのは日次スナップショットを取った時点（JST 3:00）のみ**で、
     それ以降の更新は最大24時間ぶん失われる。
   - 手順は [backup-design.md](./backup-design.md) §9 と `restore-runbook.md`（作成予定）に従う。
     **トリガー停止・FK順序・スキーマ差分の扱いを誤ると復元が失敗する**ため、必ず手順書を読んでから着手する。
   - 復元は本番へ直接流さず、先に dev で試す。
   - 失われる範囲を明示し、**合意を取ってから**実行する。
   - ⚠ **日次スナップショットの実装が完了するまでは、この手段も存在しない。**
     その場合に取れるのは論理削除の復旧（上記1）のみで、物理削除は**復旧不能**である。
     関係者への報告ではその事実をそのまま伝える。
