...
> ステータス：**調査/設計フェーズ（実装未着手）** ／ 最終更新：2026-05-20
...
**推奨：まず①で「使い始める」→ 効果を確認してから②へ。** ①は今日中に検証導線まで作れる。
②は SSO・配布の体験は良いが、Azure AD 登録（IT 部門）と Teams 管理者承認という社外依存が入る。
...
1. **今すぐ**：本メモで方針合意（①先行を推奨）
2. **①検証**：frame-ancestors 設定 → Teams タブに URL 追加 → 数名で試用（社外依存ゼロ）
3. **判断**：使われ方を見て、SSO/組織配布が要るなら②へ
4. **②本格**：manifest＋（必要なら）Azure AD SSO。相談書を Vercel 構成に更新して正式申請
