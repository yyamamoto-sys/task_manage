...
ゼロからPower Automateのフローを組む必要はない。**既にある「タスク管理アプリ｜Webhookアラート_EGG」フローを複製し、投稿先チャンネルだけ変える**のが一番簡単。

1. [Power Automate](https://make.powerautomate.com/) を開き、左メニュー「マイ フロー」→「自分と共有」タブから **「タスク管理アプリ｜Webhookアラート_EGG」** を開く
2. 画面上部のツールバーから **「名前をつけて保存」** をクリック
   → 「このフローのコピーを作成する」ダイアログが出る
3. フロー名を部署がわかる名前に変更する（例：**「タスク管理アプリ｜Webhookアラート_〇〇部」**）→「保存」
   → マイフロー一覧に新しいコピーが追加される（トリガーURLも自動的に新しく発行される）
4. 作成されたコピーを開き、フロー図の中の **「チャットまたはチャネルでメッセージを投稿する」** アクションを探す
5. そのアクション内の **「チーム」「チャネル」** のプルダウンを、通知を届けたい部署のチーム・チャンネルに変更して保存
   - コピー直後はフローが**オフ**になっていることがあるので、オンになっているか確認する（オフならツールバーの「オンにする」をクリック）
6. フロー詳細画面に戻り、トリガー（「Webhook要求を受信したときに」）を開くと **HTTP POST URL** が表示されるのでコピー
7. task_manageの管理画面 → グループ → 対象部署を編集 → 「Teams Webhook URL」欄に貼り付けて保存（super-admin限定）

これで完了。メンション解決・PJごとの振り分け等のロジックはコピー元からそのまま引き継がれるため、変更するのは**チャンネル指定の1箇所だけ**でよい。
