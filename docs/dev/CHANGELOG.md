...
#
# v3.58 「他PJから引き継ぐ」の日付基準の既定を「元PJ開始日を基準にする」に訂正（2026-08-12）
#   【経緯】v3.57で日付基準の既定を「引き継がない」にしたが、これは統括が「旧実装（〜v3.56）は
#   元PJ開始日→新PJ開始日の相対日数を常に保って日付を引き継んでいた（オプトアウト不可）」という
#   事実を把握しないまま出した誤った指示に基づくもので、結果的にv3.56以前の便利な既定挙動を
#   退行させていた。山本さんに確認し、既定を「元PJの開始日を基準にする」に戻すことが決まった
#   （Section 4のv3.49→v3.50の訂正と同種の事故）。
#   【変更内容】
#   ①既定のanchorSelectionを"none"→"project_start"に変更。モードのリセット時（引き継ぎ元PJ
#     切替時・マイルストーン基準のチェックを外したときのフォールバック先）も同様に変更した。
#   ②「元PJの開始日を基準にする」は専用の日付入力を持たせず、新PJ自身の「期間」開始日欄
#     （resolvedNewStartDate＝startDate||todayStr()。未入力なら今日＝v3.56以前のフォールバック
#     と同じ式）をそのまま新しい基準日として使う設計に変更（旧v3.57は他のアンカーと同じ専用
#     input欄を要求していたが、project_startについては新PJの開始日欄と意味が重複するため
#     入力を二重に求めない設計にした。これにより「何も選ばずに作成する」＝v3.56以前と全く同じ
#     結果になる、という最重要要件を満たす）。
#   ③マイルストーンの取り扱いを再検討し、**マイルストーンのチェックボックス自体を無効化する
#     方式（案1）を採用した**（山本さんからの判断依頼）。理由：もう一方の案（"引き継がない"でも
#     マイルストーンだけ元PJ開始日基準で相対移動する）は、"引き継がない"という選択のラベルと
#     実際の動作が食い違い、利用者にとってより紛らわしいと判断した。「基準が無ければ日付が
#     決まらないマイルストーンは作れない」ことを明示（チェックボックスを`disabled`＋行を半透明に
#     し、上に注記文を表示）する方が誠実だと判断した。「基準」ラジオ自体は無効化しない
#     （これを押すことが"none"から脱出する唯一の手段のため）。この決定に伴い
#     `buildInheritedMilestones`（`taskInheritance.ts`）はdateOffsetDays===nullのとき
#     チェックの有無に関わらず空配列を返すよう変更した（UIの無効化と実装の両方で二重に保証）。
#   ④回帰テストを追加：旧`dateSlide.ts`（`computeSlidedDate`。v3.57で削除済み）の全テスト
#     ケース（正のオフセット・負のオフセット・同日・日付無し・作業期間保持・月またぎ）を、
#     新しい「`computeInheritOffsetDays`でオフセットを求め`computeInheritedTaskDates`で
#     適用する」2段階の計算で再現し、結果が完全に一致することを固定した
#     （`inheritTaskDates.test.ts`の新設describe「旧dateSlide.ts(computeSlidedDate)との互換性」）。
#   変更ファイル：`ProjectCreateModal.tsx`（既定値・resolvedNewStartDate導入・canSaveの条件
#     緩和・マイルストーン無効化UI）。`lib/project/taskInheritance.ts`（`buildInheritedMilestones`
#     の空配列ショートサーキット）。`lib/project/__tests__/taskInheritance.test.ts`（該当テストを
#     新挙動に更新）。`lib/project/__tests__/inheritTaskDates.test.ts`（回帰テスト6件を追加）。
#     CLAUDE.md Section 8（PJ作成モーダルの行に訂正の経緯を追記）。
#   検証：`npx tsc --noEmit`エラー0／`npx vitest run`1325件全通過（1319件→1325件・+6件）／
#     `npx eslint`変更ファイルに新規エラー0／`npm run build`成功。
#   マイグレーション：追加なし。
#
# v3.59 「他PJから引き継ぐ」をステップ式UIに作り直し＋日付の選択肢を2つに整理（2026-08-12）
#   【経緯】v3.57〜58は「作成方法／引き継ぎ元PJ／日付の基準／マイルストーン／タスク／
#   メンバー／PJ基本情報」を単一画面に一気に出す設計だった。山本さんから「マイルストーンの
#   各行がチェックボックスとラジオの2つの意味を同時に持ち、片方の選択が他方を殺すため、
#   初めて見る人には何を求められているか分からない」との指摘を受け、UIを作り直した。
#   【変更内容】
#   ①日付の選択肢を3つ→2つに整理：「スケジュール間隔を引き継ぐ」（旧「マイルストーンを
#     基準にする」を改称）と「日付を引き継がない」の2択のみにし、v3.58で既定にした
#     「元PJの開始日を基準にする」は撤去した（山本さんの明示指示）。「スケジュール間隔を
#     引き継ぐ」を選ぶと、元PJのマイルストーンを1つ「基準」として選ばせ、新PJではいつに
#     置くかを入力させる。基準以外のマイルストーンは「他のマイルストーンも引き継ぐ」という
#     単純なチェックボックス1つで一括on/offする（旧実装の「行ごとのチェックボックス＋基準
#     ラジオ」という二重構造は解体した）。引き継ぎ元PJにマイルストーンが1件も無い場合は
#     「スケジュール間隔を引き継ぐ」自体を選べなくし理由を明記、既定は「日付を引き継がない」
#     にする（`lib/project/projectCreateSteps.ts`の`defaultDateStrategy`）。
#   ②画面を5ステップ（まっさらな新規作成時は2ステップ）に分割：①作成方法・引継ぎ元PJ→
#     ②日程の引き継ぎ方→③インポートタスク→④インポートメンバー→⑤名前をつけて作成
#     （確認＋PJ基本情報入力）。まっさらな新規作成を選んだ場合はステップ②〜④を飛ばし
#     「①作成方法→②名前をつけて作成」の2ステップになる。各ステップに進捗表示（`n / 総数`
#     ＋ステップ名）と、初見の利用者向けの短い案内文（具体例つき）を置いた。最終ステップ
#     以外は「作成」ボタンを出さない（誤って途中で作成されるのを防ぐ）。「戻る」で前のステップ
#     に戻っても入力内容は消えない（状態は親コンポーネントに残したまま表示を切り替えるだけ
#     のため）。
#   ③ステップ遷移と「次へ」に進めるかの判定を`lib/project/projectCreateSteps.ts`の
#     `resolveSteps(mode)`/`canGoNext(step, state)`という純粋関数に切り出しテストを書いた
#     （`projectCreateSteps.test.ts`：「まっさら」時にステップが2つになること、基準日未入力
#     では次へ進めないこと等を固定）。`canSave`（「作成」ボタンの可否）も内部で`canGoNext`の
#     schedule判定を再利用し、ロジックの重複を避けた。
#   ④モーダルの高さのガタつき対策：本文（`MODAL_BODY_STYLE`）の直下に`minHeight:"300px"`を
#     持つラッパーdivを置き、ステップごとにコンテンツの高さが大きく変わっても箱が極端に
#     縮まないようにした。Section 21の契約（`modalStyles.ts`のオーバーレイ／箱／本文／
#     フッター構造）自体は変更していない。
#   ⑤旧`dateSlide.ts`互換の回帰テスト（`inheritTaskDates.test.ts`）はUI選択肢が撤去されても
#     計算自体（`computeInheritOffsetDays`/`computeInheritedTaskDates`）は変わらないため、
#     関数レベルのテストとして維持（describeのタイトルのみ、UI選択肢撤去済みである旨に更新）。
#   変更ファイル：新規`lib/project/projectCreateSteps.ts`＋`__tests__/projectCreateSteps.test.ts`。
#     `ProjectCreateModal.tsx`（ステップ式UIへの全面書き換え。マイルストーンの行ごとの
#     チェックボックス＋基準ラジオを解体し、基準の`CustomSelect`＋一括チェックボックス1つに
#     変更）。`lib/project/__tests__/inheritTaskDates.test.ts`（回帰テストのdescribeタイトルを
#     更新。テスト内容自体は変更なし）。`CLAUDE.md`Section 8（PJ作成モーダルの行を実態に
#     合わせて更新）。`src/lib/version.ts`（3.58→3.59）。
#   検証：`npx tsc --noEmit`エラー0／`npx vitest run`全通過（既存1325件＋新設
...
#   指示を画面表示にも適用してだ・である調に書き換えていた。しかし`AdminView.tsx`の既存
#   UI文言は全面的にです・ます調で統一されており、新規分だけ変えると1箇所だけ文体が浮く
#   ことを実装時に指摘。統括が山本さんに確認し、「だ・である調で淡々と」は配布資料
#   （報告書・マニュアル・ガイド・手順書）限定のルールであり、UI文言には適用しない、
#   ただし「問いかけ・詩的表現を使わない」という禁止事項はUIにも等しく適用する、という
#   線引きが確定した。
#   【変更内容】
#   ①`AdminView.tsx`：ゲストメンバーカードの説明文をです・ます調に戻した
#     （「特定のプロジェクトのみ招待された部署外のメンバーです。部署絞り込みの対象外の
#     ため、ここに常に表示します。」＝v3.60より前の文面と同一）。
#   ②`CLAUDE.md`「UI/UX仕様」節に、UI文言の文体方針（です・ます調で統一・配布資料の
...
#
# v3.65 利用者向けバージョン履歴を全期間へ拡張・月ごとの折りたたみ表示を追加（2026-08-12）
#   背景：v3.63で追加した利用者向けバージョン履歴は「2026年7月以降のみ」を対象にしていた
#     （山本さんの指示）。今回、山本さんから「6月以前の過去のものも見られるようにしたい。
#     ただしアプリが重くなるなら不要」という依頼があり、負荷を実測した上で全期間へ拡張した。
#   調査：docs/dev/CHANGELOG.mdの見出しは176件だが、v2.21（2026-07-06）以降は既存の19エントリ
#     で漏れなく対応済みと確認できたため、追加対象はv1.0〜v2.20（2026年3月〜7月2日）のみだった。
#     このうちv1.0/v2.0（Phase設計ドキュメントの反映のみ）・v2.2/v2.3/v2.7（ホスティング確定・
#     zustand移行・楽観ロック強化等の内部アーキテクチャのみ）は利用者の操作・見た目が変わらない
#     ため対象外とし、残るv2.1・v2.6・v2.17・v2.19・v2.20の5件（同日の複数バージョンは既存の
#     方式に倣い1エントリへ統合）を追記した。日付の特定：この期間はCHANGELOG.mdの見出しに
#     日単位の日付が無いバージョンがあるため、`git log -S"<見出し文言>"`で該当テキストを
#     最初に追加したコミットの日付を採用した（v2.20は実装が2026-06-26〜07-02の複数日にまたがる
#     旨がCHANGELOG.md自身に明記されているため、完成日である07-02を採用）。
#   追加：`src/lib/releaseNotes/groupByMonth.ts`（`groupReleaseNotesByMonth()`：新しい順の
#     エントリを隣接する同月ごとにグループ化／`defaultOpenMonthKeys(monthKeys, referenceDate,
#     hasPeriodFilter)`：開いている月キーの集合を返す純粋関数。期間で絞り込み中は該当月を
#     すべて開き、絞り込みが無いときは当月・前月だけを開く）＋
#     `__tests__/groupByMonth.test.ts`（12テスト）。
#   変更：`VersionHistoryModal.tsx`のインラインだった月グルーピング処理を上記の純粋関数へ移行。
#     `openMonths`（`Set<string>`）をReact stateとして持ち、期間指定（startDate/endDate）が
#     変わるたびに既定状態へ再計算し直す。月見出しを`<button aria-expanded>`のクリック開閉に
#     変更（▶アイコンの回転で開閉状態を表示）。
#   変更：`src/lib/releaseNotes.ts`に5エントリ追記（v2.20/v2.19/v2.17/v2.6/v2.1）＋
#     v3.65自身のエントリ（今回の変更点そのもの）を先頭に追加。`src/lib/version.ts`（3.64→3.65）・
#     `CLAUDE.md`（Section 29更新・バージョン表記）。
#   検証：`npx tsc --noEmit`エラー0／`npx vitest run`全通過（1396テスト。新規12件含む）／
#     `npx eslint`新規エラー0（既存エラー・警告は今回変更したファイル以外の既存分で無関係）／
...
#
# v3.86（2026-08-20）：タスク追加「＋」FABを共通コンポーネント化し、待機中は右端へ収納・半透明にする
#   クレーム：「タスク追加のプラスボタンが、メニュー表示などで何らかのテキストと被り、
#   プラスマークより下のレイヤーの表示が見えない」への対応。
#   A（共通化）：PC版・モバイル版で完全に別JSXとして重複実装されていたFAB本体・展開メニューを
#   `src/components/layout/QuickAddFab.tsx`に切り出した。座標・展開メニューの起点はpropsで
#   受け、切り出し前の実装値（座標・サイズ・色・アニメ遅延・メニュー項目のpadding/height差）を
#   1つずつ突き合わせて一致させた。
#   B（待機中の収納・半透明）：待機中はFABを右へtranslateX(50%)（48pxの半分=24pxぶん）ずらし、
#   不透明度を0.4に下げる。カーソル接近・展開メニュー表示中・フォーカス中・タッチ端末
#   （`(hover:hover)`不成立）では常に完全表示に戻す。
#   ホバー検知は「FABの周囲に一回り大きい透明要素を重ねる」方式ではなく、window全体の
#   pointermoveでFAB本来（収納前）の中心からの距離（半径64px）を見る方式を採用した
#   （FABの位置がconsultPanelWidth等で動的に変わるため、透明要素を別途置くと同じ位置計算を
#   2箇所に持つことになりズレやすい。DOM要素を増やさずに済む利点もある）。
#   `prefers-reduced-motion:reduce`では収納アニメ（transform/opacity）のtransitionをJS側の
#   判定で無効化する（背景色のホバー変化は既存globals.cssの方針どおり止めない）。
#   C（Toastとの座標衝突解消）：`Toast.tsx`のトースト通知（旧: bottom:24px/right:24px固定）が
#   PC版FABと完全に同一座標だった。新規`src/lib/layout/fabLayout.ts`にFABの座標・サイズの
#   共有定数を集約し、Toast側はFABの高さ+余白ぶん持ち上げた値（PC:84px／モバイル:128px）を
#   この定数から算出して使う（マジックナンバーの二重管理を避ける）。
#   D（ErrorBarとの関係確認）：「履歴N件」バッジ単体はFABと数px〜十数pxの隙間があり通常は
#   重ならない（エラー行が複数積み上がると一時的に重なりうるが15秒で自動消去される一過性の
#   表示のため今回は対応対象外と判断し、その理由をコード内に明記した）。
#   E（機械チェック）：`modalStyles.test.ts`の「全画面オーバーレイ全般」チェックが、FABの
#   背景クリック用スクリム（`position:fixed;inset:0`）を新たな対象として検出したため、
#   `ErrorBar.tsx`と同型の理由（子を持たない透明スクリムのみ・クリップされうるコンテンツが
#   無い）でEXCLUDED_FILESに追加した。
#   テスト：既存1631件を壊さず全通過（新規テストファイルは追加していない・既存の
#   modalStyles.test.ts側の除外リスト追加のみ）。
#   やらないこと：DBスキーマ変更なし。
...
#
#
# v3.91（2026-08-21）：v3.86のFAB「収納・半透明」を差し戻し＋右下スタックの一元管理
#   山本さんの指示：①保存ボタンと＋ボタンが被っている（TaskSidePanelのフッターの上にFABが
#   重なっていた）②＋ボタンの位置・透明度をv3.86以前に戻してほしい（収納・半透明の撤去）
#   ③その上でFABを削除/保存ボタンのゾーンを避けるよう少し上げる④ショートカットボタンとの
#   衝突にも注意。
#   A（v3.86の収納・半透明を撤去）：`QuickAddFab.tsx`からFAB_IDLE_OPACITY/
#   FAB_IDLE_TRANSLATE_X/FAB_NEAR_RADIUS_PXの3定数・pointermoveリスナー・isNear/isFocused
#   state・useHoverCapable()/usePrefersReducedMotion()の2フックを削除。待機中も常に
#   不透明度1・オフセット0。タッチ端末分岐も収納自体が無くなったため削除。共通コンポーネント化
#   （PC/モバイル統合）自体はv3.86のまま維持（山本さんの指示は位置と透明度のみ対象）。
#   B（根本原因の特定）：TaskSidePanelフッター・FAB・ショートカットボタン・Toastがそれぞれ
#   独立にbottom値を手書きしており、v3.90時点で既に3件の重なりが存在した：PC＝フッター
#   [0,40)×FAB[24,72)・ショートカット[100,128)×Toast[84,120)、モバイル＝ショートカット
#   [128,156)×Toast[128,164)（bottom同値）。修正前に実際にこの3件を検出できることを
#   確認済み（旧fabLayout.tsの値を使った使い捨てテストで実測）。
#   C（`src/lib/layout/fabLayout.ts`→`bottomStack.ts`に改名・発展）：「右下に積み上がる
#   要素のスタック」を1ファイルで一元管理する設計に変更。積み順（下から）＝TaskSidePanel
#   フッター（PCのみ）／ボトムナビ（モバイルのみ）→FAB（→展開時はFAB展開メニュー）→
#   ショートカットボタン→Toast。各要素のbottomは「1つ下の要素のbottom+高さ+クリアランス
#   （12px）」の式で算出し、手書きの数値を並べない。高さが要る要素（フッター・ショートカット・
#   Toast・FAB展開メニューのモバイル項目）は対応コンポーネント側もこの定数を明示heightとして
#   使う（TaskSidePanel.tsx/MainLayout.tsx/Toast.tsx/QuickAddFab.tsx）。
#   D（FABの新しい位置）：PC＝52px（フッター高さ40px+クリアランス12px。旧24pxから28px
#   上げた・フッターを避ける最小限）。モバイル＝68px（旧値と同じ・ボトムナビ+クリアランス）。
#   PC側は「TaskSidePanelが実際に開いているか」に関わらず常にこの位置にする（開閉のたびに
#   動くとちらつくため）。
#   E（ショートカット・Toastの退避）：ショートカットボタンはFAB本体の上端+クリアランスに
#   置き、FABメニュー展開時はメニューの上端+クリアランスまで押し上げる（旧仕様と同じ考え方）。
#   ToastはFABメニューの開閉に関わらず出現しうるため、ショートカットが最も高くなる
#   「FABメニュー展開時」の位置を基準に常に静的に確保する（通常時のToast位置は旧84pxから
#   280pxへ大きく上がるが、FABメニュー展開中も含めてどの状態でも重ならないことを優先した）。
#   F（機械チェック新設）：`src/lib/layout/__tests__/bottomStack.test.ts`（新規5件）。
#   PC/モバイル×FABメニュー展開中/非展開中の4パターン全てで、bottomStack.tsの定数から
#   計算した占有区間がどの2つも重ならないことを検査する。実装前にv3.90時点の値で実際に
#   3件のテストが赤くなることを確認済み。
#   G（CLAUDE.md Section 43を全面改訂）：収納・半透明を撤去したこと・その理由（利用者の
#   意見）・右下スタックの一元管理設計・新しいグランドルール（右下に要素を追加するときは
#   必ずこのスタックに載せる）を明記。
#   既存1658件を壊さず、合計1663件が全通過（新規5件）。
#   やらないこと：DBスキーマ変更なし。ErrorBarの扱い（一過性のため対象外）はv3.86のまま変更なし。
#
# v3.92（2026-08-21）：Toastの位置を「FABの真上まで」に是正し、不変条件を「操作を妨げない
#   こと」へ再定義
#   統括からの差し戻し：v3.91は`TOAST_BOTTOM_*_PX = SHORTCUTS_BOTTOM_FAB_OPEN_*_PX + 28 + 12`
#   （PC=280px）と、ショートカットボタンがFABメニュー展開時に退避する最も高い位置を基準に
#   Toastを常時静的確保しており、900px程度の画面で下から1/3ほど浮いた不自然な位置になって
#   いた。「どの2つも重ならない」という不変条件を、重なっても実害の無いペア（Toast↔
#   ショートカットボタン）にまで一律に適用したのが原因。
#   A（Toastの新しい位置）：`TOAST_BOTTOM_PC_PX = FAB_BOTTOM_PC_PX + FAB_SIZE_PX +
#   STACK_CLEARANCE_PX`（=112px。SHORTCUTS_BOTTOM_PC_PXと同じ式・同じ値）。モバイルも同じ
#   考え方（=128px）。数値が一致するのは偶然ではなく、どちらも「FABの直上に一段だけ載る」
#   という同じ設計だから。
#   B（不変条件の再定義）：このスタックが守るべきは「操作を妨げないこと」であって「一切
#   重ならないこと」ではない。要素を「FAB・FAB展開メニュー＝利用者が押したい主要な操作
#   ボタン（隠してはいけない）」と「ショートカットボタン＝常設だが補助的なaffordance（数秒
#   隠れても実害無し）」の2種に分け、Toast（z-index最前面・数秒で自動消去される一過性表示）
#   はFABさえ隠さなければよいという整理にした。
#   C（テストの直し方）：`bottomStack.test.ts`の「どの2つも重ならない」検査を、ペアごとに
#   「重なってはいけない」「重なってよい」を明示する形へ変更。`allowOverlap(a, b, reason)`
#   が理由の文字列（20文字以上）を必須にし、無しでは登録できない（例外を投げる）ようにした。
#   ALLOWED_OVERLAPSに2ペア登録：①Toast↔ショートカット（本件の本体）②Toast↔FAB展開メニュー
#   （Toastが静的な位置のためFABメニュー展開時にも重なりうる。メニュー項目はクリック直後に
#   メニュー自体を閉じてから処理を実行する設計のため、あるアクション結果のToastとそのFAB
#   メニューが同時に開いたまま重なることは通常起きない、という理由で許容）。Toast↔FAB本体は
#   引き続き重ならないことを機械チェックで守る（許容リストに入れていない＝構造的に重ならない
#   ことをテストが検証する）。
#   D（許容リストの健全性チェック）：ALLOWED_OVERLAPSの各ペアが少なくとも1つのシナリオで
#   実際に重なっていることを検証するテストを追加（不要な許容登録＝将来の抜け穴を防ぐ）。
#   E（ErrorBarとの関係を再確認）：Toastを280pxから112px（PC）/128px（モバイル）へ下げた
#   ことで、ErrorBarの最大積み上げ状態（帯の高さ100px超）との位置関係がv3.90時点に近い値へ
#   戻ったことを確認した。ErrorBarは引き続き一過性のため右下スタックの対象外（変更なし）。
#   F（CLAUDE.md Section 43を更新）：Toastの新しい位置・不変条件の再定義・許容リストの
#   仕組みを明記。グランドルールのチェックリストに「許容したい場合はALLOWED_OVERLAPSへ理由
#   付きで登録する」を追記。
#   既存1663件を壊さず、合計1665件が全通過（bottomStack.test.tsに2件追加＝許容ペアの健全性
#   検査・FABメニュー展開時のクリアランス検査）。
...
#
# v3.101（2026-08-26）：個人OKRに月全体・四半期全体の振り返りを記録する「全体」タブを追加（CLAUDE.md Section 24 Step Q）
#   依頼：「我々は月末に月次面談をGMと行い、一緒に振返りとコメントをいただくので、その月
#   全体の自己評価とGM評価も受取ります。個別KRに加え、全体のことも記録できるタブがあると
#   嬉しい」。山本さんが選んだ設計判断：記録項目は全体の自己評価%/GM評価%/全体の振り返り
#   本文（本人）/GMコメントの4つのみ（面談日・来月への申し送りは無し）。全体%は参考値を
#   機械計算で提示し確定は手入力。AI下書きを用意する。単位は月・四半期の両方。
#   🔴 唯一マイグレーション1本：`supabase/migrations/20260826_add_personal_period_reviews.sql`
#   （新テーブル`personal_period_reviews`。山本さんが手動適用）。一意制約は部分ユニーク
#   インデックス2本（WHERE period_kind='month'／'quarter'）——`UNIQUE(...,month)`だと
#   Postgresの NULL 非等価によりquarter行（monthは常にNULL）の重複を防げないため。RLSは
#   personal_krsと同じ「member_idを直接持つため親を辿らない」流儀（新規ヘルパー関数なし）。
#   🔴🔴 W2（最重要）：テーブル未適用でも「全体」タブだけが案内を出し、KRタブは完全に
#   従来どおり動く。usePersonalOkrUiStoreに`periodReviews`/`periodReviewsLoaded`/
#   `periodReviewsLoading`/`periodReviewsError`という既存のkrs系stateと完全に独立した
#   state群を新設。`loadPeriodReviews()`はfetch失敗を`periodReviewsError`へ積むだけで
#   例外を投げ直さない（krs等には一切触れない）。実装前にtry/catchを外した状態で
#   実際に回帰テストが赤くなることを確認済み（`personalOkrUiStore.test.ts`）。
#   `schemaChecks.ts`に`personal_period_reviews_table`を追加。
#   UI：`PersonalOkrView.tsx`のKRタブ列の先頭にsentinel（`OVERALL_TAB_ID`）で「全体」タブを
#   追加（KR一覧自動選択effect・ensureKrDetailLoaded effectの対象外にする分岐を追加）。
#   選択すると新規`PersonalOverallView.tsx`が縦2ブロック（月全体・四半期全体）を新規
#   `PersonalPeriodReviewBlock.tsx`（periodKind違いで2回使う。コピペしない）で描画する。
#   参考値：新規`src/lib/personalOkr/periodReviewReference.ts`
#   （computeWeightedAverage/computeMonthlyAverage/computePeriodReference・純粋関数）。
#   月＝Σ(自己評価%×weight_pct)÷Σ(weight_pct)、四半期＝各KRの3か月平均を先に求めてから
#   同じ加重平均に使う。weight_pctが全て0または記入済みKRが0件ならnull（0除算回避）。
#   入力欄4つは明示保存（v3.87/93/96の作法）。dirty判定は既存の`monthReviewForm.ts`の
#   computeMonthReviewDirty/parseEvalPctInputをそのまま再利用（判定ロジックを二重化
#   しない）。編集可否は`isMonthEditable`と同じ思想。四半期側は新設
#   `isQuarterEditable()`（quarterMonths.ts）。`unsavedEditorRegistry`への登録も実装済み
#   （`PersonalPeriodReviewBlock.tsx`が月・四半期の各インスタンスでuseId+register/
#   unregisterを呼ぶ。MonthReviewBlock.tsxと同型）。
#   AI下書き：新規`src/lib/ai/personalOkrPeriodReviewDraftExtractor.ts`
#   （AIIntent="okr-personal-period-review-draft"・model=claude-sonnet-4-6・
#   max_tokens=3072）＋新規`src/lib/personalOkr/periodReviewDraftContext.ts`（文脈組み立て。
#   既存のcomputeMonthWeekSegments/buildWeekCards/computeWeekCardsLinkedTasks/
#   computeReviewMaterialを再利用）。WEEKLY_IS_OPTIONAL_NOTICEを埋め込み。数値は書かせない
#   （出力の型がreview_text/basisのみ）。546対策：各800字クリップ＋総文字数上限（目安
#   10000字）超過で①計画4欄の要点→②タスク内訳→③GMコメント→④振り返り本文の順に
#   フィールド種別ごと一括削除（決定的順序・テストで固定）。UI（生成→提示→反映の3段階。
#   DBへは書かない）は新規`PersonalOkrPeriodReviewDraftModal.tsx`
#   （PersonalOkrPlanDraftModal.tsxと同型）。AI生成履歴テーブルは作っていない（山本さんの
#   依頼どおり。ローカルstateのみで閉じれば消える）。
#   Step 0で確認：①既存マイグレーション作法（update_updated_at共有トリガー・確認クエリ）
#   を踏襲。②personalOkrUiStoreの各アクションはupsertByIdによるマージ型更新（丸ごと
#   置換ではない）。③KRタブの帯・unsavedEditorRegistry登録は既存の作法をそのまま踏襲。
#   ④schemaChecks追加はkind:"table"の1エントリで足りる。
#   新規テスト：periodReviewReference.test.ts（13件）・periodReviewDraftContext.test.ts
#   （16件・546対策の削減順序が決定的であることを含む）・
#   personalOkrPeriodReviewDraftExtractor.test.ts（9件）・quarterMonths.test.tsに
#   isQuarterEditableを5件追加・personalOkrUiStore.test.tsに「全体」タブ用のゲスト分岐・
#   テーブル未適用時の回帰テストを6件追加・guardedNavigateCoverage.test.tsに「全体」タブ
#   切替パターンを追加（実装前に未ガード状態で実際に赤くなることを確認済み）。
#   `npx tsc --noEmit`0・`npx vitest run`全1847件通過・`npm run build`成功。
...
#
# v3.109（2026-09-17）：PJ基本情報の編集を部署メンバー全員へ開放（利用者からの要望）
#
# * 利用者から「プロジェクトの名前が変えられない」という声。原因は
#   canEditProjectBasicInfo() が部署管理者・全社スーパー管理者に限っていたこと。
# * 🔴 調査で判明：この制限はUIにしか無かった。projects のRLS（projects_group）は
#   部署スコープのみを条件にしており、DB側は元々「同じ部署なら誰でもUPDATE可」。
#   UIとDBで条件が食い違っていた（APIを直接叩けば一般メンバーでも更新できた）。
# * 山本さんの判断でDB側に合わせてUIを緩めた。開放範囲は基本情報タブ全体
#   （名前・目的・貢献メモ・オーナー・期間・色・ステータス）。
# * 呼び出し元は ProjectSettingsModal.tsx と MainLayout.tsx の2箇所のみで、
#   関数1つの変更で完結。サイドバー「⋮」の完了/アーカイブ/復帰も同時に開放された
#   （基本情報タブでステータスを変えられるのに「⋮」からは不可、という不整合を作らないため）。
# * 引数（_members / _currentUser）は将来また権限を絞るときのために意図的に残した。
# * ゲストへの影響なし（buildProjectRowMenuItems が空配列を返し、saveProject も
#   ゲスト分岐でメモリ上の更新に留まる）。
