...
> **🔴 2026-08-12・山本さんの決定で変更（v3.55）：発火はOKRモード起動時（KRタブを開いた
> とき）の自動発火 → 明示ボタンを押したときのみに変更した。** 当初案（本項の初版）は
> 「対象KRタブを開いたときのみ発火」だったが、実装後（Phase 3後半・v3.52）に実機で使うと
> KRが複数本あるたびに切替ごとにAI呼び出しが走り、切替に何十秒もかかって実用に耐えなかった
> （山本さんの報告）。以下は現行の設計。

- **発火は明示ボタン（AheadBlock.tsx。未解析なら「✦ 見立てを出す」・解析済みなら「再解析」に
  文言が切り替わる1つのボタン）を押したときのみ。** KRタブを開いた・月を切り替えただけでは
  呼ばない。cron・全員一律のバッチ解析もやらない（モードを開かない人・ボタンを押さない人には
  1トークンも発生させない）。
- **保存済みの解析結果（`personal_kr_outlooks` の直近行）の読み込みは自動のまま。** 別端末・
  別セッションでも過去の見立てが消えて見えないよう、KRタブ（当月）を開いたときに
  `ensureOutlookLoaded()`（DBを1回読むだけ・ゼロトークン）だけを自動で呼ぶ。AI呼び出し本体
  （`analyzePersonalKrOutlook`）はこの経路からは呼ばない。
- **`input_fingerprint` が前回と一致したら呼ばない。** ハッシュに含めるもの：対象KRに紐づくタスクの `updated_at` の最大値／週の目標状態と `self_rating`／月次計画の `imported_at`／メモの最終 `updated_at`／現在の週番号。ボタンを押したときにこの判定を通す（未解析でキャッシュが無ければ判定に関わらず呼ぶ・解析済みで明示的に押した場合はforce:trueで一致していても必ず呼ぶ）。
- **粒度は開いているKRタブ1本だけ。** 全KR分をまとめて解析しない。
- **「これからの見立て」と「バンドのAI判定」は1回の呼び出しにまとめる。** 別々に呼ぶと倍かかる。
- 結果は `personal_kr_outlooks` に永続化するので、**別端末・別セッションでも再解析されない**。実質これがいちばん効くトークン削減。
- `AIIntent` に新タグを追加済み（Section 6-1b・16）：`okr-personal-outlook`。振り返り下書きは**別枠の呼び出し**にする（月末に1回・明示ボタン・プロンプトも頻度も別）。案：`okr-personal-review-draft`。
...
### 個人OKRビュー
**最上部に個人KRごとのタブ**（ウェイト付き。自己研鑽・全社共通も1本ずつ）。
🔴 **月の選択は「対象期」行（年・四半期の隣）に一元化し、KRタブをまたいで共有する**（v3.55・
2026-08-12。当初案では月タブをKRタブ内に持たせる想定だったが、KR切替時に月がリセットされる
不具合が実機で発生したため、対象期側に移した。CLAUDE.md Section 24 Step J）。
タブ内は このKRの内容（折りたたみ・取込）→ 今月の計画（取込・**過去月も編集可**）→ **週の目標状態（W1〜W5・◯△✕で自己評価。評価すると週の色が変わる。任意の補助機能）** → **これから** → 迷ったらAIに聞く → **振り返り**（review_text／自己評価%／GM評価%／GMコメント。過去月も編集可）→ メモ。

- **「今日の焦点」は作らない**（毎日見るものではない・山本さん指摘）。
- **中心は「これから」**。過去は、これからの判断を変えるときにだけ出す（「先月と同じ理由でつまずいている」等）。
- 🔴 **2026-08-26（v3.96）以降：過去月は読み取り専用ではなく編集可**（計画欄・週カード・バンド決定・振り返り欄すべて。`isMonthEditable()`。CLAUDE.md Section 24 Step N）。「これから」のAI解析・AIパネルのみ当月限定のまま。
...
**4** | ✅ **完了（2026-08-20・v3.83／2026-08-26・v3.96で下書きの保存先を統一）** 月末の振り返り下書き（明示ボタン・別 `AIIntent="okr-personal-review-draft"`）。新テーブル`personal_kr_review_drafts`（AI生成はINSERT）。過去月でも生成可、自己評価％・バンド数値はAIに書かせない設計。**v3.96で「編集を保存」の保存先を`personal_kr_review_drafts.edited_text`から月の`personal_kr_months.review_text`へ一本化**（振り返りブロック新設・過去月の編集解放と同時実施）。詳細はCLAUDE.md Section 24 Step M・Step N | あり |
