...
**2** | Kintone取込（PDF／貼り付け → 抽出 → 人が確認 → 登録）。既存の取込基盤（`FileAttachButton` / `docxText` / `invokeAI`）を流用 | 抽出のみ |
**3** | `personal_kr_outlooks` ＋ 「これから」＋ バンドAI判定 ＋ フィンガープリント ＋ AIパネルのOKR版 | 中核 |
**4** | 月末の振り返り下書き（明示ボタン・別 `AIIntent`） | あり |
**5** | `okr_knowledge_docs`（要点インデックス方式）＋ ナレッジ閲覧 ＋ 個人KR設計支援（既存の10ステップ対話フローを内蔵） | あり |
**6** | グループビュー刷新 ＋ 照合（言 vs 実）＋ 取込の統合 | あり |
...
- Kintone取込の入力形式（PDFエクスポート／画面コピペ／CSV）。実物で試して確かめる
