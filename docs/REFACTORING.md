...
| M11 | 親子階層ロジックの一元化 | 中 | `taskHierarchy.ts`が正本のはずが、GanttView`orderTasksHierarchically`・ListView`buildRows`/`derivedByParentId`に同種の集計が重複（2026-07-06のパフォーマンス改修で追加した分含む）。集計ヘルパーをtaskHierarchy.ts側へ寄せる |
| M12 | スタイル定数の共通化 | 中 | `inputStyle`/`ghostBtnStyle`/`chipStyle`等がほぼ同内容で13ファイルに重複定義。モーダルオーバーレイ（`position:fixed,inset:0`）も22ファイルに散在。`src/styles/`配下に共通定数モジュールを作成 |
| M13 | KanbanView内蔵`AddTaskModal`とQuickAddTaskModalの統合 | 中 | タスク追加フォームが2系統並存し入力項目の仕様差（開始日・メモ・子タスク一括入力）が生まれている。M9より費用対効果が高い可能性 |
| M14 | ListViewモバイルカード行のReact.memo化 | 中 | 2026-07-06のパフォーマンス改修はPC表の行（ListTaskRow）のみ対応。モバイルカード行は未対応 |
...
| AI 紫の全置換 | 47箇所（2026-07-06時点）の hex を `var(--color-ai-*)` に置換（基盤は `globals.css` で完了済み・現在 `var(--color-ai` の利用は0件） | 機械的 sweep・次回候補 |
