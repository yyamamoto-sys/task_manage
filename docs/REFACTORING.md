...
- **`lib/list/groupSummary.ts`の`computeGroupSummary`に、既知のM33（`GanttView.tsx`のPJ別/
  ...
  **追記（2026-07-22）**：この設計判断は同日、山本さんの方針確定（cancelledは完了扱い・on_holdは
  未完了扱いのまま）を受けて解消済み。詳細は下記「観察のみ・次回候補として記録」節のM33欄・
  CLAUDE.md v2.76参照。
...
- ~~**M33**~~：**解消済み（2026-07-22・CLAUDE.md v2.76）**。PJ別/ToDo別ビューのPJ・ToDoグループ見出しの進捗%（`done/total`によるバー幅・パーセント表示）がcancelledを完了扱いしていない（`status==="done"`のみで算出）問題を、山本さんの方針確定（cancelledはdoneと同じ完了扱い・on_holdは引き続き未完了扱い＝`allChildrenTerminal`と同じ基準）を受けて一括解消。`src/lib/taskMeta.ts`に共有ヘルパー`isCompletedForProgress(status)`を新設し、対象4箇所（`GanttView.tsx`のPJ別/ToDo別グループ進捗%・`DashboardView.tsx`の`pjProgress`/`krProgress`/`tfTaskStats`/`todoProgress`・`groupSummary.ts`の`computeGroupSummary`＝ListView/Kanban共有）を統一。葉タスクの進捗フィル`leafProgressFraction`（v2.45）もcancelledをdoneと同じ1（100%）扱いに統一。親タスク自身の`done`算出（`parentProgress`/`buildParentDerivedMap`。個別タスクバー進捗フィル・ListView親行表示に使用）はM33が対象化した4箇所に含まれておらずM11の残課題として別管理されているため、今回は意図的に対象外のまま（詳細はCLAUDE.md v2.76参照）。
...
| ~~M24~~ | ~~`appStore.ts`の`quarterlyKrTaskForces` state・`addQuarterlyKrTaskForce`/`removeQuarterlyKrTaskForce`アクション、`store.ts`の対応するfetch（2箇所）・`insertQuarterlyKrTaskForce`/`deleteQuarterlyKrTaskForce`関数が、2026-05-26のTF→四半期判定モデル移行（`quarterly_kr_task_forces`テーブルベース→`TaskForce.quarter`列ベース）後、UIからの呼び出し元が丸ごと0件のまま残置されている（`quarterlyKrTaskForces` stateはfetchするだけで読むコンポーネントが無い）~~ | — | **完了（2026-08-10 / v3.38・OKRモード再設計 Phase 1 Step C）** フロントエンドの死蔵コード（`appStore.ts`のstate・2アクション・`store.ts`のfetch/insert/delete）を削除。**DBテーブル（`quarterly_kr_task_forces`）自体はDropしていない**（物理削除禁止・CLAUDE.md Section 4。過去データが残っている可能性があるため）。`quarterly_objectives`は別テーブルで、OKR PDF取込（`OkrImportModal`）が「四半期OKR」選択時に記録用の骨組みを今も1件作成する書き込み経路が残っているため撤去していない（取込機能を壊すリスクを回避）。両テーブルとも`schema.sql`に「死蔵」の明記コメントを追加、`QuarterlyKrTaskForce`型定義（`types.ts`）も死蔵注記を追加して残置 |
| ~~M25~~ | ~~`SetupWizard.tsx`が新規作成するメンバーに`group_id`を一切設定せず、`appStore.saveMember`の`get().currentGroupId ?? undefined`フォールバックに委ねているが、SetupWizard実行時点では`currentGroupId`が一度も更新されない（`null`のまま）。マルチテナンシーのRLS（`members_group`：`group_id = current_member_group_id() OR current_member_is_super_admin()`。`current_member_group_id()`は`email = auth.email()`でmembers行を検索）の下では、該当emailにマッチするmembers行がまだ存在しない真に新しいテナントの最初のメンバー作成時、`current_member_group_id()`も`current_member_is_super_admin()`もNULL/falseとなりINSERTがRLSに拒否される可能性が高い~~ | — | **完了（2026-07-22 / CLAUDE.md v2.84）** SECURITY DEFINER関数2本（`is_system_bootstrapped()` / `bootstrap_first_group_and_member()`。`supabase/migrations/20260722_add_onboarding_bootstrap.sql`）を新設。未登録ユーザーは`AccessDeniedScreen`へ、真の初回セットアップ（members 0件）のみ`SetupWizard`が部署名入力＋ブートストラップ関数経由で部署・最初のsuper-admin作成まで面倒を見る設計に変更。詳細はCLAUDE.md Section 1.6「オンボーディング経路」参照 |
