...
> ステータス：**設計レビュー中**（実装未着手）。レビューOK後にフェーズ順で実装する。
...
現状：AI が HTML レポートを自動生成 → localStorage 保存のみ。変更後：**AI下書き → 人が手修正 → 承認依頼 → 承認者が承認/差戻し**。

- AI が下書きを生成（③分析結果を素材として引く）。`status = draft`。
- 人が編集（マークダウン or リッチテキスト）。`status = edited`。
- 「承認依頼」を押すと `status = submitted`。
- 承認者が **承認**（`status = approved`、`approved_by`/`approved_at` 記録）または **差戻し**（`status = rejected`、コメント付き → 編集に戻る）。
- 承認者：当面は誰でも承認可（10名フラット設計）だが、誰が承認したかは記録する。将来「KR/TFのリーダーのみ承認可」に絞る余地あり。
- 確定（approved）レポートの「学び」が ④→① 引き継ぎで次週ノートに差し込まれる。
...
CREATE TABLE kr_reports (
  ...
  content      text NOT NULL DEFAULT '',                -- 本文（編集可）
  status       text NOT NULL DEFAULT 'draft' CHECK (status IN ('draft','edited','submitted','approved','rejected')),
  review_note  text NOT NULL DEFAULT '',                -- 差戻し時のコメント
  created_by   text NOT NULL,
  approved_by  text,
...
- **④ レポート作成**：KR＋週セレクタ → 「✨ AI下書き生成」（③を素材に） → 編集エリア → 「承認依頼」。承認待ち一覧 → 承認者が「承認 / 差戻し（コメント）」。承認済みは読み取り専用＋コピー/投稿用テキスト出力。
...
| **C** | レポート確認・承認制 | `kr_reports` テーブル＋store＋KrReportPanel改修（下書き→編集→承認依頼→承認/差戻し）＋③から素材を引く | レポートが承認なしには確定されず、承認者・承認日時が記録される |
...
- [ ] `okr_tf_analyses` の保持件数（最新N件 or 全件）
- [ ] `kr_reports.mode` の種別整理（週次レポート／月次レポート／総務提出用 をどう分けるか。既存 `kr-report-howto.md` の「チェックイン向け／ウィン向け」モードとの関係）
- [ ] 承認者の範囲（当面：誰でも可。将来：TF/KRリーダー限定？）
- [ ] ①ノートの「①先週動かした仮説」へ前週「③次の一手」をどう差し込むか（全文コピー／参照表示のみ／編集者が選んで取り込み）
- [ ] サイクルナビの「対象週」の決め方（その時点の月曜を自動／手動選択も可）
- [ ] freeform セッションは循環ステップ②のどこに位置づけるか（②の一形態として扱う想定）
- [ ] スマホ表示（OKRモードは現状PC前提の画面が多い。新画面もPC優先でよいか）
