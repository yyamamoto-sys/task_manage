# CLAUDE.md — グループ計画管理アプリ 設計ドキュメント v3.87
#
...
- **サイドパネルのタスク切替**：他の行をクリックして別タスクを選ぶと、旧タスクにdirtyな変更があれば「「◯◯」に保存していない変更があります。保存してから次のタスクへ移動しますか？」／選択肢「保存して移動する」「破棄して移動する」の2択で確認する。**「切り替えをキャンセルして元のタスクに留まる」は実装していない**：3つの呼び出し元（`GanttView`/`KanbanView`/`ListView`）がそれぞれ独自に選択中タスクIDを持っており、「切り替えを取り消す」には呼び出し元へ選択を戻す新しいコールバックが要る。「切り替え自体は進む前提で、保存するか破棄するかだけを聞く」の2択で「黙って破棄しない・黙って保存しない」という要件は満たせるため、3ファイルへの越境変更は見送った。
