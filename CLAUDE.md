...
### Step Qの是正（v3.102・2026-08-26・山本さんの適用前レビューで検出）

v3.101のマイグレーション（未適用のまま）に2件の欠陥があり、山本さんが適用する前に統括のレビューで検出・修正した。
...
## 43. タスク追加「＋」FABの共通化（v3.86）＋右下スタックの一元管理（v3.91・v3.92で不変条件を是正・2026-08-21）

### v3.86：共通コンポーネント化（維持）

クレーム「タスク追加のプラスボタンが、メニュー表示などで何らかのテキストと被り、プラスマークより下のレイヤーの表示が見えない」への対応として、PC版・モバイル版で完全に別のJSXとして重複実装されていたFAB本体・展開メニュー（旧`MainLayout.tsx`）を`src/components/layout/QuickAddFab.tsx`に切り出した。座標・展開メニューの起点（PC＝AI相談パネル幅ぶん動く／モバイル＝固定）は`isMobile`propと`isConsultOpen`/`consultPanelWidth`/`isConsultResizing`propで受ける。**この共通化はv3.91でも維持している**（山本さんが差し戻しを求めたのは次項の「収納・半透明」であって、コンポーネント統合ではない）。`data-tour-id="fab"`（将来のツアーが参照する可能性がある予約属性）はPC側のみ維持している。

### v3.91：v3.86の「待機中は右端へ半分収納＋半透明にする」を撤去

**クレーム（利用者の意見）**：①保存ボタンと＋ボタンが被っている（タスクのサイドパネルを開いた状態で、フッターの「保存」ボタンの上に＋ボタンが重なっていた）。②＋ボタンの位置と透明度は元に戻してほしい（＝v3.86で入れた「右端へ半分収納」「待機中は半透明」をやめる）。③その上で、＋ボタンが「削除」「保存」ボタンのあるゾーンに被らないよう、少し上へずらす。④その際、ショートカットボタンと被らないように気をつける。

- `QuickAddFab.tsx`から、収納・半透明に関わる全て（`FAB_IDLE_OPACITY`/`FAB_IDLE_TRANSLATE_X`/`FAB_NEAR_RADIUS_PX`の3定数、`pointermove`リスナー・`isNear`/`isFocused`state・`useHoverCapable()`・`usePrefersReducedMotion()`の2フック・収納時のtransform/opacity分岐）を削除した。待機中も常に不透明度1・オフセット0で表示する。
- タッチ端末向けの分岐（hoverできない環境では収納しない）も、収納自体が無くなったため不要になり削除した。
- FABの◇マーク開閉時の回転（`isFabMenuOpen`のとき`rotate(45deg)`）はv3.86の収納・半透明とは別機能（＋を✕に見せる演出）のため維持した。

### v3.91：右下に積み上がる要素のスタックを一元管理する（`src/lib/layout/bottomStack.ts`。旧`fabLayout.ts`から改名）

**根本原因**：v3.86〜v3.90時点は「TaskSidePanelのフッター（削除／保存ボタン）」「FAB」「ショートカットボタン」「Toast」がそれぞれ独立に自分のbottom値を手書きしており、片方を動かすたびに別のものと当たっていた。統括が確認したところ、v3.90時点で以下の重なりが実際に存在した：
- **PC・通常時**：TaskSidePanelフッター（bottom 0〜約40px）とFAB（bottom 24〜72px）が24〜40pxの範囲で重なる（今回のクレーム①の実体）。
- **PC・通常時**：ショートカットボタン（bottom 100〜128px）とToast（bottom 84〜120px）も100〜120pxの範囲で重なっていた（クレームには無かったが横断確認で発覚）。
- **モバイル・通常時**：ショートカットボタン（bottom 128px）とToast（bottom 128px）がbottom同値で完全に重なっていた。

この3件は、修正前に実際にテスト（後述）で検出できることを確認済み（詳細はテストの節を参照）。

**設計**：「画面右下（PC）／画面下端（モバイル）に積み上がる要素のスタック」を`src/lib/layout/bottomStack.ts`に定義した。積み順（下から上）は次のとおり：
...
各要素の`bottom`は「1つ下の要素の`bottom` + 高さ + クリアランス（`STACK_CLEARANCE_PX`=12px）」の式で算出し、手書きの数値を並べない。高さが必要な要素（TaskSidePanelのフッター・ショートカットボタン・Toast本体・FAB展開メニュー項目）は、対応するコンポーネント側もこの定数を明示`height`として使う（`TaskSidePanel.tsx`のフッター・`MainLayout.tsx`のショートカットボタン・`Toast.tsx`のトースト本体・`QuickAddFab.tsx`のモバイル版メニュー項目）。**これにより、フッターの padding やボタンサイズを変えたら FAB の位置が自動で追随する**（高さの見積もりとレイアウト実体が同じ定数を参照するため、乖離が起きない）。

主な値（v3.92・2026-08-21時点）：

| 要素 | PC bottom | モバイル bottom | 高さ |
|---|---|---|---|
| TaskSidePanelフッター（PCのみ） | 0 | — | 40px |
| ボトムナビ（モバイルのみ） | — | 0 | 56px |
| FAB本体 | 52px（40+12） | 68px（56+12・旧値と同じ） | 48px |
| FAB展開メニュー（3項目） | 102px（52+48+2） | 118px（68+48+2） | 126px／136px |
| ショートカットボタン（通常時） | 112px（52+48+12） | 128px（68+48+12・旧値と同じ） | 28px |
| ショートカットボタン（FAB展開時） | 240px | 266px | 28px |
| Toast | 112px（52+48+12） | 128px（68+48+12） | 40px |

**FABの上げ幅（クレーム③）**：「サイドパネルのフッターの上端＋クリアランス」に置く（52px＝フッター高さ40px＋クリアランス12px）。旧位置（24px）からの上げ幅は28pxで、フッターを避けられる最小限に留めた（必要以上に上げると画面中央寄りに浮いて目立つため）。**PC側は「TaskSidePanelが実際に開いているかどうか」に関わらず常にこの位置にする**（開閉のたびにFABの位置が動くとちらつく・「開いた瞬間だけ重なる隙」が生まれるため）。TaskSidePanelはPC・タブレット専用でモバイルには出ない（`TaskSidePanel.tsx`冒頭コメント。モバイルの編集はTaskEditModal＝中央寄せの全画面モーダルで、この右下スタックには参加しない）ため、モバイル側のFAB位置は変更していない（旧68pxのまま＝ボトムナビの上端+クリアランス）。

**ショートカットボタン・Toastの退避（クレーム④）**：ショートカットボタンはFAB本体の上端+クリアランスに置き、FABメニュー展開時はメニューの上端+クリアランスまで押し上げる（旧仕様と同じ考え方・数値だけ追随）。

**🔴 v3.91→v3.92：Toastの位置を「FABの真上まで」に是正した（このスタックが守るべき不変条件の再定義）。** v3.91は「ショートカットボタンが最も高くなるFABメニュー展開時」を基準にToastを常に静的確保する設計にしており、その結果Toastの通常位置がv3.90時点（84px）から280px（画面の下から約1/3）まで押し上げられていた。「どの2つも重ならない」という不変条件を、重なっても実害の無いペアにまで一律に適用したことが原因（統括の指摘）。

このスタックが本当に守るべき不変条件は**「操作を妨げないこと」であって「一切重ならないこと」ではない**。要素は次の2種に分かれる：
- **FAB（と、その展開メニュー3項目）**：利用者が押したい主要な操作ボタン。隠してはいけない。
- **ショートカットボタン**：常設だが補助的なaffordance。数秒間Toastに隠れても実害が無い（通知が消えれば元に戻る）。

Toastはz-index最前面に出るうえ数秒で自動消去される一過性の表示のため、「FABを隠さない」ことだけを守り、「ショートカットボタンと重ならない」ことまでは要求しない。v3.92でToastを`FAB_BOTTOM_*_PX + FAB_SIZE_PX + STACK_CLEARANCE_PX`（＝ショートカットボタンの通常位置と同じ式）に変更し、**Toast⇔ショートカットボタンの重なりは意図した許容**として`bottomStack.test.ts`の`ALLOWED_OVERLAPS`に理由付きで登録した。ToastはFABメニュー開閉によらず静的な位置のままのため、**FABメニュー展開時はToast⇔FAB展開メニューも重なりうる**（同じ理由＝メニュー項目はクリック直後にメニュー自体を閉じてから処理を実行するため、あるアクション結果のToastとそのFABメニューが同時に開いたまま重なることは通常起きない）。これも`ALLOWED_OVERLAPS`に登録した。**Toast⇔FAB本体（利用者が押したい主要な操作ボタン）は重ならないことを維持している**（Toastの位置自体がFABの真上＝FAB本体の上端+クリアランスのため、構造的に重ならない）。

**AI相談パネルが開いているときの`right`の避け方（`consultPanelWidth + 24`）は現状の挙動を維持**した（変更していない）。

### `TaskSidePanel`のフッター高さを明示`height`にした

フッター（🗑削除／保存ボタン）のdivに`height: ${SIDE_PANEL_FOOTER_HEIGHT_PX}px`（40px）を明示した。従来は`padding`とボタンサイズによる自然な高さ（実測約37〜38px）に任せていたが、明示することで「フッターの見た目＝FABが避けるべき高さ」を1つの数値に固定した（見た目上の差は数px以内で、現状の見た目を変えない）。

### ErrorBarとの関係（v3.86からの記録・v3.92で再確認・変更なし）

「履歴N件」バッジ単体（エラー表示が無い状態）はFABと数px〜十数pxの隙間があり、通常は重ならない。エラー行が複数（最大5件）積み上がると帯の高さが伸びFAB・展開メニューと重なりうるが、エラーは15秒で自動消去される一過性の表示のため、右下スタックの対象外と判断している（`ErrorBar.tsx`のコメントに理由を明記）。**v3.92でToastのbottomを280pxから112px（PC）/128px（モバイル）へ下げたことで、ErrorBarの最大積み上げ状態（帯の高さ100px超）との位置関係がv3.90時点に近い値へ戻ったことを確認した**（Toastは84px→112pxで、ErrorBarとの相対関係は元々許容していたリスクの範囲内。この判断自体は変更していない）。

### 機械チェック（v3.91新設・v3.92で拡張）

`src/lib/layout/__tests__/bottomStack.test.ts`が、`bottomStack.ts`の定数から各要素の占有区間`[bottom, bottom+height)`を計算し、PC・モバイル×FABメニュー展開中／非展開中の4パターン全てで、**`ALLOWED_OVERLAPS`（理由付きの許容リスト。理由の説明が無いと登録できない＝`allowOverlap()`が例外を投げる）に載っていない重なりが無いこと**を検査する。**実装前にv3.90時点の値（PC：フッター/FAB・ショートカット/Toast、モバイル：ショートカット/Toast）に対して実際にこのロジックを走らせ、3件とも赤くなることを確認済み**（本Section冒頭の「根本原因」参照）。v3.92では追加で、`ALLOWED_OVERLAPS`の各ペアが実際に少なくとも1つのシナリオで重なっていること（＝不要な許容登録が残っていないこと）も検査する。

### 🔴 グランドルール：右下に要素を追加するときは必ずこのスタックに載せる

画面右下（PC）／画面下端（モバイル）に新しい固定要素（バッジ・ボタン・通知等）を追加するときは、bottom値を手書きせず、必ず`src/lib/layout/bottomStack.ts`のスタックへ新しい段として追加し、`bottomStack.test.ts`の重なり検査に組み込むこと。**重なりを許容したい場合は、bottom値の調整で無条件に回避しようとせず、まず「その要素は利用者が押したい操作ボタンか、隠れても実害の無い補助的な要素か」を判断し、後者同士の組み合わせに限って`ALLOWED_OVERLAPS`へ理由付きで登録すること。**

---

## 44. グランドルール：タスク編集面（モーダル／サイドパネル）は明示保存にする（必須・v3.87・2026-08-20）

クレーム「タスクの詳細を追加した後、自動で保存されて×で消すの、本当に反映できてるか分からなくて不安。自動保存されているかどうかに関わらず、自分で保存ボタンを押したら画面が閉じるという流れの方が安心できる」への対応。

### 何を変えたか

`TaskEditModal.tsx`（タスク編集モーダル）・`TaskSidePanel.tsx`（右サイドパネル）の**`form`／`sidebarForm`（このタスク自身のフィールド）は、以後 保存ボタン／Enter／Ctrl(Cmd)+Enter でのみ保存する**。以前は`form`変更を600msデバウンスして自動保存し、×やタスク切替時に保留分をその場でフラッシュする設計だった（編集消失バグへの対処としてコメント付きで入っていた）が、この自動保存の仕組み自体が「保存されたか分からず不安」というクレームの原因のため、丸ごと廃止した。

- **保存ボタンは常時表示**。押すと保存して閉じる（モーダル）／押しても閉じない（サイドパネル。元々✕でしか閉じない設計）。dirty（保存済み内容と差分がある）でなければ`disabled`＋明度を下げ、`title`で「変更はありません」と伝える。dirty判定は`src/lib/taskEditPayload.ts`の`computeFormDirty()`（値の比較。担当者・タグは順序を無視した集合比較）。
- **Enterで保存できるが、画面は閉じない**。単一行の`<input>`（タスク名等）でのEnterのみ対象。`<textarea>`（メモ・コメント）のEnterは改行のまま。**Ctrl(Cmd)+Enterはどこからでも保存**（captureフェーズで拾い、タグ入力欄・CustomSelectの検索inputが先にpreventDefaultしていても必ず発火する）。日本語入力の変換確定のEnter（`e.nativeEvent.isComposing`）は必ず無視する。
- **保存済み（未変更）のときは保存ボタンをdisabled＋明度を下げる**。既存の`SaveIndicator`（✓保存しました／保存失敗のバッジ）とは役割を分離した：保存ボタン＝「押せるか・保存中か」、`SaveIndicator`＝「結果（成功／失敗）」の伝達のみに専念する（両方が別々に「保存中」を語らないようにするため、`SaveIndicator`はidle/savingのとき何も表示しない）。

### なぜ自動保存をやめたか

自動保存自体が悪いわけではなく、**「今どういう状態なのか利用者から見えない」**ことが不安の原因だった（保存済みかどうかの手がかりが薄いSaveIndicatorだけでは足りなかった）。明示保存にすると「押した／押していない」「押せる（変更あり）／押せない（変更なし）」が常に視覚的に分かるため、同じ不安を構造的に無くせると判断した。

### なぜjoin系（タスクフォース・追加プロジェクト・先行タスク・子タスクの付け外し）だけは即時のままか

これらは`form`を経由しない別テーブルへのadd/remove操作（`addTaskTaskForce`/`removeTaskTaskForce`・`addTaskProject`/`removeTaskProject`・`addTaskDependency`/`removeTaskDependency`・`TaskSidePanel`の`applyChildren`/`detachChild`）で、**操作した瞬間に結果（チップの増減）が画面に見える**。これらまで保存ボタンの対象にすると「チップは増えたのに保存ボタンを押すまで本当に登録されたか分からない」という、今回解消したかった不安の別形態を新たに生む。この線引きは両ファイルの冒頭コメント・各セクションのインラインコメントに明記した。

### close時の暗黙フラッシュは廃止した

以前は×ボタン・タスク切替のたびに、保留中の編集をその場で`saveTask`する「フラッシュ」処理があった（デバウンス発火前のアンマウントで編集が消える実データロスバグへの対処としてコメント付きで入っていた）。明示保存に変えた今、このフラッシュを残すと「保存ボタンを押していないのに閉じたら保存された」という、今回のクレームの裏返しが起きるため削除した。代わりに、閉じる・タスクを切り替える経路すべてでdirtyなら`confirmDialog()`により確認する：

- **×ボタン・オーバーレイクリック（モーダルのみ。サイドパネルはSection 21の対象外でクリックアウトサイド自体が無い）**：「保存していない変更があります。破棄して閉じますか？」／選択肢「破棄して閉じる」「編集に戻る」の2択。**3択（保存して閉じる、を含む）にはしなかった**：保存したいだけなら常時表示の保存ボタンが既にあるため、確認ダイアログ側にまで持たせる必要は薄いと判断した（実装・文言とも複雑になる）。
- **サイドパネルのタスク切替**：他の行をクリックして別タスクを選ぶと、旧タスクにdirtyな変更があれば「「◯◯」に保存していない変更があります。破棄して次のタスクへ移動しますか？」／選択肢「破棄して移動する」（confirm・tone="danger"）「保存してから移動する」（cancel）の2択で確認する。**「切り替えをキャンセルして元のタスクに留まる」（＝呼び出し元の選択そのものを差し戻す確認）は実装していない**：3つの呼び出し元（`GanttView`/`KanbanView`/`ListView`）がそれぞれ独自に選択中タスクIDを持っており、「切り替えを取り消す」には呼び出し元へ選択を戻す新しいコールバックが要る。「切り替え自体は進む前提で、保存するか破棄するかだけを聞く」の2択で「黙って破棄しない・黙って保存しない」という要件は満たせるため、3ファイルへの越境変更は見送った（ただし保存が実際に失敗した場合の選択の巻き戻しはv3.88で対応。Section 45参照）。**confirm=破棄・cancel=保存という割り当て（v3.88）**：`ConfirmModal`は背景クリックで必ず`cancel`（false）扱いになるため、うっかり背景をクリックしても編集内容が保存される側に倒れるよう、「破棄」をtone="danger"のconfirmボタン（赤・押すのに意図が要る）に、「保存」を安全側のデフォルト（cancel・背景クリック）に割り当てている。
- **Escキー**：現状どちらの画面にもEscapeで閉じる機能自体が実装されていない（調査済み）。今回は新しい閉じる経路を追加していないため対応不要（存在しない機能に警告を追加する必要は無い）。

### finalized_mentions（@メンション通知）の扱い

以前は「閉じた時だけ確定保存する」という別仕様だった（`TaskEditModal.tsx`のみ・サイドパネルは元々メンション機能自体が無い）。明示保存に変えたことで「閉じるまで通知が飛ばない」という仕様の前提（close時フラッシュ）が消えたため、**保存のたびに常に`finalized_mentions`をペイロードへ含める**形に寄せた（`extractMentions(form.comment)`を毎回計算）。変化が無ければ同じ値を書き込むだけで実害は無く、「変更があったときだけ」という条件分岐（旧`mentionsEqual`比較）自体が不要になった。

### 2-5：競合の窓が広がる問題への対策（`saveTask`自体には手を入れない）

明示保存でフォームを開いたままの時間が延びると、Realtimeで他人の更新が届いても`saveTask`内部の`expectedUpdatedAt`（store の現在値から読む）がその新しい値に更新済みになり、競合が検知されないまま自分の内容で上書きしてしまう抜け穴が広がる（Section 5参照）。**`saveTask`の共通ロジック・Section 5の契約は一切変更していない。** 代わりに両画面それぞれの中で：

- フォームを開いた時点／保存に成功した時点の`updated_at`を`baselineUpdatedAtRef`に保持する。
- 保存直前に、その時点で分かっている`updated_at`（storeから購読済みの値）と`baselineUpdatedAtRef`を比較し、異なれば「このタスクは他の人が更新しました。あなたの内容で上書きしますか？」を`confirmDialog()`で確認してから保存する（拒否したら保存しない）。
- 開いている最中に他人の更新が届いたことは、控えめな警告バナー（⚠ 他のメンバーがこのタスクを更新しました…）で画面上にも知らせる。

### 機械チェック

`src/components/task/__tests__/explicitSaveNoDebounce.test.ts`が、`TaskEditModal.tsx`/`TaskSidePanel.tsx`に`form`/`sidebarForm`を依存配列に持つ`useEffect`が`setTimeout`と組み合わさっている（＝デバウンス自動保存の実装フィンガープリント）ことを検出するテスト。実装前にv3.86時点（デバウンスが実在した状態）のソースに対して実際に走らせ、正しく検出できる（＝赤くなる）ことを確認済み。検出ロジック自体が壊れていないことを合成フィクスチャでも固定している。

---

## 45. `TaskSidePanel`：タスク切替時の保存失敗で編集内容が消える不具合の修正（必須・v3.88・2026-08-20）

v3.87のリリース確認で発覚：`TaskSidePanel.tsx`のタスク切替useEffect（Section 44参照）で、利用者が「保存して移動する」を選んだのに`saveTask`が失敗した場合、`catch`でtoastを出すだけでそのまま新タスクへの切替（`setSidebarForm(nextForm)`等）まで進んでしまい、旧タスクの編集内容（`sidebarForm`・baseline）を失っていた。「自動保存されているか分からず不安」というクレームへの対応の趣旨（編集が失われないと確信できること）と正面から矛盾するため、v3.88で以下のとおり修正した。

### 修正内容

- **保存に失敗したら切替を中止する。** `catch`ブロックで`onSwitchFailed(prevTask.id)`（後述）を呼んだ上で`return`し、`setSidebarForm`等の新タスクへの切替処理を一切実行しない。`sidebarForm`／`baselineFormRef`／`formTaskRef`は旧タスクの値のまま保持される（dirtyな内容は消えない）。
- **失敗は永続的なエラー帯で表示する。** `setSaveStatus("error")`／`setSaveError(formatErrorForUser(...))`を呼ぶことで、既存の`{saveStatus === "error" && saveError && (...)}`帯（Section 15の流儀）がそのまま表示される。トーストのように数秒で消えることはない。
- **選択状態のずれ対策：`onSwitchFailed`コールバックを新設（必須prop）。** 呼び出し元（`GanttView`/`KanbanView`/`ListView`）は行クリック時に自分の選択state（`editingTaskId`/`selectedTaskId`）を即座に新タスクへ進める設計のため、切替失敗後もそのままだと「パネルは旧タスクを表示し続けるのに、呼び出し元の選択状態は新タスクのまま」というズレが生じる。これを避けるため、`TaskSidePanel`の`Props`に`onSwitchFailed: (previousTaskId: string) => void`を**必須**で追加し、3つの呼び出し元すべてで`id => set(Editing|Selected)TaskId(id)`（選択を旧タスクへ戻す）を実装した。必須にしたのは、省略された場合にこのズレが静かに再発するのを防ぐため。
- **自己ループ防止のガード。** `onSwitchFailed`によって`taskId`が旧タスクIDへ戻ると、useEffectの依存配列`[taskId]`が再度変化し、同じuseEffectがもう一度発火する。この2回目の発火は「実質的には切替が起きていない（旧タスクへ戻っただけ）」ため、`if (taskId === formTaskRef.current?.id) return;`という早期returnをuseEffectの先頭に追加し、無意味な再ダイアログ表示や状態の再設定を防いでいる。

### `ConfirmModal`の背景クリック挙動の再確認結果（統括の依頼に対する回答）

`ConfirmModal.tsx`にEscapeキーで閉じる仕組みは無い（`keydown`リスナー自体が存在しない）ことを確認した。**ただし背景クリックは`onClick={() => handleClose(false)}`により必ず`cancel`（`false`）扱いで閉じる。** これ自体はconfirmDialogの全19+箇所で以前からの仕様であり、`tone="danger"`の削除確認等では`false`＝「キャンセル（削除しない）」という安全側の意味に一致しているため問題は無かった。

しかし、v3.87で新設したタスク切替の確認ダイアログだけは、`confirmLabel="保存して移動する"`（true）／`cancelLabel="破棄して移動する"`（false）という割り当てだったため、**背景を誤ってクリックすると「破棄して移動する」が無言で選ばれてしまう**危険な組み合わせになっていた（Escapeでの事故ではなく、背景クリックでの事故）。v3.88でこの1箇所のみ`tone="danger"`・`confirmLabel="破棄して移動する"`・`cancelLabel="保存してから移動する"`に入れ替え、背景クリック（＝`cancel`）が常に安全側（保存してから移動する）に倒れるようにした。他の4箇所の新設confirmDialog（Section 44参照：モーダル/パネルの閉じる確認2箇所・2-5の上書き確認2箇所）は、`cancel`側が元々「何もしない（安全）」を意味する設計だったため対象外（問題なし）。

### 監査結果：他に編集消失経路が無いか

- **`TaskEditModal.tsx`に同種の経路は無い。** `handleClose`は「dirtyなら確認し、確認が取れたら（＝discardの意思表示があったら）discardしてから閉じる」という設計で、閉じる直前に暗黙の保存を試みる分岐が無い（保存を試みるのは明示的な`handleSave`だけで、これは閉じる操作を伴わない）。よって「保存を試みて失敗したのに閉じてしまう」という経路自体が存在しない。`TaskSidePanel.tsx`の`handleClose`（✕ボタン）も同型で問題なし。
...
## 48. クレーム「内容を変更したのに保存ボタンに🚫が出て押せない」への対応（必須・v3.93・2026-08-21）

### 調査結果：(A)/(B)/(C)分類

`TaskSidePanel.tsx`・`TaskEditModal.tsx`の操作可能なコントロールを全て洗い出し、(A) `form`/`sidebarForm`を変更しdirty経由で保存ボタンから保存される・(B) formを通らず即時保存される・(C) 画面上で変わるのに保存もされずdirtyにもならない、の3つに分類した。

| コントロール | 分類 | 備考 |
|---|---|---|
| タスク名・状態・優先度・担当者・プロジェクト・開始日/終了日・工数・メモ/コメント・タグ（モーダルのみ） | (A) | `computeFormDirty`（`taskEditPayload.ts`）が全フィールドを比較。既存の`taskEditPayload.test.ts`で網羅済みであることを確認済み |
| 階層モード（単独/子タスク/親タスクの切替ボタン） | (A)相当 | ボタン自体はローカルUI状態（`hierarchyMode`）の切替のみで、保存が要る変化は必ず`parent_task_id`（sidebarFormの一部）を経由する。モードだけ切り替えて中身を選ばない操作は「保存すべき差分が無い」ため、dirtyにならないのは正しい挙動（バグではない） |
| 親タスク選択・「子タスクを選ぶ」 | 親タスク選択=(A)（`parent_task_id`）／子タスクを選ぶ=(B)（`applyChildren`が対象タスクへ直接`saveTask`） | |
| 先行タスク・追加プロジェクト・タスクフォース（追加/解除） | (B) | `addTaskDependency`/`removeTaskDependency`等。join/unjoinで結果が即座に画面に見える設計（v3.87の線引き） |
| 削除 | 対象外 | 確認ダイアログ付きの独立操作 |

**(C)に該当するコントロールは見つからなかった。** `computeFormDirty`とTaskEditFormStateの全11フィールドの対応漏れも無い（新設した機械チェック`computeFormDirtyFieldCoverage.test.ts`で固定。実装前に`parent_task_id`の比較行を一時的に削除し、実際にテストが赤くなることを確認済み）。

**baselineの不正更新経路も見つからなかった。** `baselineFormRef`を書き換えている箇所は両ファイルとも「タスク切替時の初期化」「保存成功時」の2箇所のみで、Realtime更新や(B)系の即時保存操作からbaselineが書き換わる経路は無い。

### 実際の原因：(B)の即時保存が完全に無言だった

(C)が無い一方、(B)（タスクフォース・追加プロジェクト・先行タスク・子タスクの付け外し）の追加/解除は、既にDBへ保存済みであるにもかかわらず、成功時に一切のフィードバックが無かった（`applyChildren`のみtoastがあったが、TF/追加PJ/先行タスクの追加解除・`detachChild`は無言）。利用者が「変更した」と感じた操作の多くが実はこの(B)群であり、そのあと`form`側は無変更のまま保存ボタンにカーソルを合わせて🚫を見た、という体験が「変更したのに保存できない＝壊れている」の実体だったと判断した。

### 対応

1. **(B)群すべてに、既存の`SaveIndicator`（✓バッジ）を流用したフィードバックを追加した。** `flashImmediateSaved()`/`runImmediateSave()`（両ファイルにそれぞれ実装）が、即時保存が成功するたびにヘッダーの保存インジケータを1.5秒だけ「✓」表示にする。メイン保存（`handleSave`）が`"saving"`中なら上書きしない。エラーは呼び出し先（`appStore.ts`）が既に`showToast`で通知済みのため、ここでは何もしない。
2. **未変更時の保存ボタンの見せ方を「押せない禁止ボタン」から「保存済み」表示に変えた。**
   - `cursor: "not-allowed"` → 未変更時は`"default"`、保存中は`"wait"`（🚫を完全に廃止）
   - ラベル：未変更時は「保存」→「✓ 保存済み」
   - `title`：「変更はありません」→「保存済みです。変更するとこのボタンが押せるようになります。」
   - ボタン自体は引き続き`disabled`のまま（押せないこと自体は変えていない。「押せない理由の伝え方」だけを変えた）

### 機械チェック（新設）

`src/lib/__tests__/computeFormDirtyFieldCoverage.test.ts`：`TaskEditFormState`インターフェースのフィールド名集合と、`computeFormDirty`が`current.xxx`で参照しているフィールド名集合をソース走査で突き合わせ、完全一致することを検査する。将来フィールドを追加してcomputeFormDirty側の比較を足し忘れると（＝今回と同種の「画面上で変わったのに保存できない」バグの再発）、このテストが赤くなって気づける。

---

## 49. 拡大率・フォント設定でFAB/保存ボタンがズレる問題への対応（必須・v3.94・2026-08-21）

### 何が起きていたか

v3.91/v3.92で右下スタック（Section 43）を一元管理する`bottomStack.ts`を作ったが、土台の値（`SIDE_PANEL_FOOTER_HEIGHT_PX`等）は「padding・ボタンサイズからの計算による見積もり」のままだった。ブラウザの拡大率・最小フォントサイズ設定・OSの表示スケール・フォント差異により実際の描画サイズが変わりうるため、「人によっては＋ボタンと保存ボタンが依然として被る」というクレームが（v3.91の対応後も）再発した。値を微調整して回避しても、別の拡大率・フォント設定の利用者でまた同じ問題が起きる（見積もりである限り再発し続ける）と判断し、今回は見積もり依存そのものを減らす方向で対応した。

### 2-1（本命）：FABをTaskSidePanelの横へ退避させる

`TaskSidePanel`の開閉状態と実際の幅（`panelWidth`。ドラッグでユーザーが変更できる値）を、新設した`src/stores/uiLayoutStore.ts`（zustand）で共有した。`TaskSidePanel`はマウント時に`isTaskSidePanelOpen=true`、アンマウント時に`false`をセットし、`panelWidth`が変わるたびに`taskSidePanelWidth`を更新する。`MainLayout.tsx`はこのストアを購読し、`QuickAddFab`へ新設した`extraAvoidWidthPx`（TaskSidePanelが開いていればその幅、閉じていれば0）propとして渡す。`QuickAddFab`は既存のAI相談パネル回避ロジック（`isConsultOpen ? consultPanelWidth : 0`）に、この`extraAvoidWidthPx`を単純加算する。

- **両方が同時に開く場合の扱い**：加算する。AI相談パネルとTaskSidePanelは画面右側に横並びで両方同時に開きうる（`MainLayout.tsx`の描画順は [mainContent（TaskSidePanelを含む）][AI相談パネル]）ため、両方の幅を避けなければ衝突が起きる。
- **`TaskSidePanel`はPC・タブレット専用でモバイルには出ない**（Section 43既出の前提のまま）ため、モバイル向けFABには`extraAvoidWidthPx`を渡していない（影響なし）。

**縦位置（v3.91で52pxへ上げていたもの）は24pxへ戻した。** 理由：横方向でFABが完全にTaskSidePanelの外へ出るため、フッターとの縦の衝突は構造的に起きなくなった（衝突は「同じ横位置にいるときだけ」発生するため）。「少し上へずらす」指示（v3.91）はフッターとの衝突を避けるための手段であり、衝突が構造的に起きなくなった以上、目的は既に達成されている。これに伴い`FAB_BOTTOM_PC_PX`を`SIDE_PANEL_FOOTER_HEIGHT_PX + STACK_CLEARANCE_PX`（52px）から、v3.86以前と同じ`24px`の固定値に戻した（ショートカットボタン・Toastのbottomもこの値からの積算のため連動して下がり、v3.90時点の値＝84pxに一致する）。

これに伴い、`sidePanelFooter`（TaskSidePanelフッター）は`bottomStack.ts`の1次元（縦方向のみ）の重なりモデルから外した。`bottomStack.test.ts`はbottom座標と高さだけを見る1次元の検査であり、「横に退避しているから実際には重ならない」という2次元の関係を表現できない（footerとfabが同じ横位置にいる前提が崩れたため、この2要素間の「重ならないこと」はこのテストの対象外とし、後述のランタイム実測チェック（2-3）に委ねる）。

### 2-2：残る関係（FAB↔ショートカット↔Toast↔ボトムナビ）は(c)を選んだ

これらは既に「高さの見積もり値を、対応するコンポーネント自身の明示`height`としても使う」設計（v3.91）になっており、FAB(48px固定の円)・ショートカットボタン・Toast本体はいずれも`bottomStack.ts`の定数をそのまま`style.height`に使っている＝見積もりと実体は既に同じ値を参照している。

(a)（ResizeObserver実測）を検討したが、以下の理由で見送った：
- これらの高さは既に固定`height`のCSSボックスであり、ResizeObserverで測っても`bottomStack.ts`の定数と同じ値が返るだけで、実質的な追加情報が無い。
- 拡大率・最小フォントサイズで実際に起きうる問題は「固定`height`のボックスから、内部のテキスト/ボタンが`overflow`しはみ出す」ことであり、これは親要素のボックス自体のサイズ（ResizeObserverが測るもの）ではなく、はみ出した子要素の描画位置に起因する。ResizeObserverは検知できない種類のズレのため、(a)は今回の問題を解決しない。
- `Toast`は`App.tsx`直下、`FAB`/`ショートカット`は`MainLayout.tsx`と、コンポーネント境界をまたぐ実測の受け渡し（測る側→使う側の反応的な状態共有）が必要になり、実装コストの割に上記の理由で効果が薄い。

(b)（1つのfixedなflexコンテナ+gap）も検討したが、Toast(z10000)・ショートカット(z140)・FAB(z60)はモーダル/フォーカスの重なり順の都合で意図的にz-indexが分かれており、同じコンテナにまとめるとこの重なり順の意図を壊すリスクがあるため見送った。

よって**(c)**：明示`height`による最小保証は維持しつつ、`STACK_CLEARANCE_PX`を12px→16pxへ引き上げた。ただしこれは「見積もりを少し大きくする」だけであり、今回のクレームの構造（値を足しても別の設定でまた起きる）そのものへの解決にはならないと認識している。この関係性における実質的な再発防止策は、次の2-3のランタイム実測チェックに委ねる。

### 2-3：開発ビルド限定のランタイム重なり検出（今回の再発防止の本体）

`src/lib/layout/devOverlapCheck.ts`を新設した。`TaskSidePanel`フッター・FAB本体・ショートカットボタン・Toast・モバイルのボトムナビに`data-bottom-stack="..."`属性を付与し、開発ビルド（`import.meta.env.DEV`）でのみ、`MainLayout`のマウント時に0.5秒間隔で各要素の`getBoundingClientRect()`を取得し、`bottomStack.test.ts`と同じ`ALLOWED_OVERLAPS`（Toast×ショートカット・Toast×FABメニュー）を除く組み合わせが実際に画面上で重なっていたら、要素名・重なり幅（px）を`console.warn`に出す。

**山本さんが気づく方法**：開発サーバー（`npm run dev`）で画面を開き、ブラウザの拡大率やOSの文字サイズ設定を変えながらタスク詳細を開閉する。実際に重なりが起きた瞬間、DevToolsのコンソールに`[bottomStack] 実測で重なりを検出: "fab" × "toast"（約12×8px）`のような警告が出る。本番ビルドでは`import.meta.env.DEV`が`false`になるため何も実行されず、利用者には見えない。

### CLAUDE.md新設ルール：右下スタックの土台に描画サイズの見積もり値をハードコードしない

`bottomStack.ts`の値は**v3.91→v3.94の2回、同じ「見積もりと実物のズレ」ですり抜けた**（1回目：v3.86のFAB位置固定値がフッターと衝突／2回目：v3.91で「少し上へ」対応した後も、拡大率・フォント設定次第でなお衝突しうるクレームが再発）。理由は、ブラウザの拡大率・最小フォントサイズ設定・OSの表示スケール・フォントの行高差により、CSSで指定した`height`と実際に描画される見た目上のサイズが一致しない利用者が一定数いるため。**値を足して回避する対応（v3.91の「28px上げる」）は、対応した瞬間の環境でしか正しさを保証できず、別の環境の利用者で必ず再発する。**

今後、右下（PC）・画面下端（モバイル）に新しい要素を積む・既存の関係を調整するときは：
- まず「本当に見積もりが要るか」を疑い、**位置関係そのものを構造的に無くせないか**（今回のFABの水平退避のように）を最優先で検討する。
- 見積もりに頼らざるを得ない場合は、`bottomStack.test.ts`のような定数どうしの整合性テストだけでは「見積もりが実物と合っているか」を検査できないことを踏まえ、`devOverlapCheck.ts`のような実測ベースの検査を併設する。
- クリアランスを増やすだけの対応は暫定策であり、恒久対応として報告しないこと。

---

## 50. グランドルール：右下スタックの要素に固定`height`を使わない（必須・v3.95・2026-08-21）

### v3.94の2-2判断が見落としていたこと

v3.94の2-2は「FAB/ショートカット/Toastは既に`bottomStack.ts`の定数と一致する明示`height`を持っているのでResizeObserverは無意味」と判断して(c)（クリアランスを少し増やすだけ）を選んだ。**この判断は循環していた**：定数と一致する`height`を持っているのは、定数を`height`としてそのまま使う設計にしたからであり、**その固定`height`自体が「人による設定への弱さ」の原因**だった。v3.94の報告書は「min-font-sizeが固定高さの箱の内容をあふれさせる」ことを正しく認識していながら、「検出できない」と書いただけで直していなかった。

固定`height`は、v3.91の「見積もり値のハードコード」・v3.91〜v3.94の「定数どうしの検算しかしないテスト」に続く、**同じ論点が3回すり抜けた3つ目の形**だった：
1. v3.91：見積もり値をハードコードして少し足すだけの対応
2. v3.91〜v3.94：`bottomStack.test.ts`が定数どうしの整合性しか検査せず、実物とのズレを検知できない
3. v3.94：固定`height`そのものが「人による設定で中身が切れる」原因だと認識しながら、「検出できない」とだけ書いて放置した

固定`height`は「ズレ」（要素どうしが重なる）ではなく、**「切れ」**（要素の中身が箱に収まらずクリップされる）という別の症状で同じ根本原因（拡大率・最小フォントサイズ設定・OSの表示スケールに弱い数値のハードコード）を再現する。**そして「切れ」はv3.91〜v3.94の重なり検査（`bottomStack.test.ts`・`devOverlapCheck.ts`）では原理的に検出できない**（重なり検査は「2つの要素が同じ場所にいるか」しか見ないため、1つの要素の中身がその箱からあふれているだけの状態は対象外）。

### 対応（v3.95）

- **文字を含む右下スタックの要素は、固定`height`をやめて`minHeight`にした。** 対象：`TaskSidePanel`のフッター（`SIDE_PANEL_FOOTER_MIN_HEIGHT_PX`）・ショートカットボタン（`SHORTCUTS_BUTTON_MIN_HEIGHT_PX`）・Toastの各項目（`TOAST_ITEM_MIN_HEIGHT_PX`）・モバイルのボトムナビ（`BOTTOM_NAV_MIN_HEIGHT_MOBILE_PX`）・FAB展開メニューの3項目（`FAB_MENU_ITEM_MIN_HEIGHT_PC_PX`/`_MOBILE_PX`）。**FAB本体（＋アイコンのみの48×48固定正円）は文字を含まないため対象外**（固定サイズのまま）。定数名に`MIN_`を付け、「最低保証の高さ」であることを明示した。
- **残る「本当の縦の依存」を洗い出した結果、統括の整理どおり2つで正しかった**（詳細はSection内の実装報告参照）。FAB本体・ショートカットボタン（通常時）・Toastの`bottom`は、この2つの実測値から`bottomStack.ts`の純粋関数（`computeFabBottomMobile`・`computeFabMenuTop`・`computeAboveFabBottom`等）で導出する設計にし、`src/stores/uiLayoutStore.ts`に`mobileBottomNavHeightPx`/`fabMenuHeightPx`を追加、測る側（`MainLayout.tsx`・`QuickAddFab.tsx`）が`ResizeObserver`で書き込む。
- **`devOverlapCheck.ts`に「あふれ検出」を追加した。** `data-bottom-stack`属性を持つ各要素の`scrollHeight > clientHeight`を見て、はみ出していれば要素名・不足px数を`console.warn`する（重なり検出とは独立の別チェック）。同じ問題を毎ポーリングごとに連呼しないよう、警告済みキーを保持し解消されたら削除する（再発したら再度警告する）。
...
## 59. 🔴 グランドルール：ソース走査型の機械チェックは「コメントに書いた文字列」で無力化される（必須・v3.114・2026-09-18）

### 何が起きたか

`ListToolbar.tsx`（v3.114で新設）の冒頭に、設計意図コメントとして
`` Portal する要素には `pointerEvents:"auto"` を明示する（Section 51） `` と書いた。

`src/components/common/__tests__/floatingPanelContract.test.ts` は、`createPortal` と
`document.body` を含む `.tsx` を**ソース文字列として走査**し、`/pointerEvents:\s*"auto"/`
にマッチするかを見る。**コメント行もソース文字列**なので、このコメントだけでマッチが成立し、
**実コードから `pointerEvents: "auto"` を削除してもテストは緑のまま**だった。

実装者が「意図的に壊して赤くなるか」を確認したことで発覚した。コメントを言い換えて修正し、
再度わざと壊して赤くなること／戻すと緑になることを確認済み。

### なぜ危険か

この検査は Section 51 の業務停止級の不具合（担当者ドロップダウンがクリックもホイールも
受け取らない）の再発防止そのものである。**検査が緑であることを根拠に「守られている」と
判断する運用**なので、無力化に気づけないと再発防止の本体が失われる。

しかも**無力化する側に悪意も不注意も要らない**。「ルールを守ったことをコメントで説明する」
という良い習慣がそのまま検査を殺す。Section 22（マイグレ追加時は検査項目も1行足す）と同じで、
検査そのものの健全性は誰も検査していなかった。

### グランドルール

1. **ソース走査型の機械チェックを新設・変更したとき、および走査対象のファイルを新設したときは、
   必ず「意図的に壊して赤くなること」「戻すと緑になること」を1回確認すること。**
   `feedback_regression_test_must_fail_before_fix`（再発防止テストは修正前のコードで落ちることを
   確認させる）と同じ原則を、走査型テストにも適用する。**緑であることは何の根拠にもならない。**
2. **走査型の検査が探すパターンそのものを、コメント・文字列リテラル・テストの説明文に書かないこと。**
   書く必要があるときは言い換える（例：`pointerEvents` を「ポインタイベントの明示」と書く）。
3. **新しい走査型テストを書くときは、可能なら行頭の `//` `/*` `*` で始まる行を除外してから
   マッチさせること。** 既存テストを一斉に書き換える必要はないが、新設するものはこの形にする。

### 同型の落とし穴がある既存テスト（2026-09-18時点の棚卸し）

いずれも「ソースを読んで文字列/正規表現で検査する」方式であり、同じ手口で無力化されうる。
**該当ファイルにコメントを書くときは、検査が探す語をそのまま書かないこと。**

- `src/components/common/__tests__/floatingPanelContract.test.ts`（今回の発生元）
...
- `src/lib/__tests__/changelogVersion.test.ts` / `version.test.ts` / `computeFormDirtyFieldCoverage.test.ts`

---

## 60. 表示の圧縮方針：拡大して使う利用者への対応（v3.114・2026-09-18）

### 前提（これを取り違えると逆効果になる）

ブラウザの拡大は**CSS px を物理的に大きくする**＝ビューポートが CSS px 換算で縮む。
ツールバーやサイドバーの縦の占有は中身の高さで決まる固定量なので、**拡大すると
「固定占有 ÷ ビューポート高さ」の比率だけが上がり**、一覧に使える行数が激減する。

🔴 **拡大して使う人が欲しいのは「大きい文字」である。`fontSize` を下げる方向の圧縮は
利用者の目的そのものを潰すので、やってはいけない。** v3.114では `fontSize` を1箇所も
変更していない。
...
### ツールバーをポップオーバーへ集約するときの型（v3.114で確立）

- 4つ以上のポップオーバーを作るなら、**共通コンポーネントを1つ作って使い回す**（Section 51 の
  教訓＝同じコードを複数箇所にコピペしない、がそのまま当てはまる）。
- **開いているメニューは単一 state（`openMenu: MenuId | null`）で持つ。** 各ポップオーバーが
  自前の boolean を持つと、同時に2つ開く状態を作れてしまう。
- 🔴 **ポップオーバーの中に `CustomSelect` を入れ子にしないこと。** `CustomSelect` は
  `createPortal(document.body)` する別のポップオーバーであり、親の「外側クリックで閉じる」
  判定がその Portal を外側と見なすため、**セレクトを開いた瞬間に親が閉じる**。選択肢が
  多くて検索が要る場合は、親パネルの中に素の `input` ＋リストを直接描画する。
- トリガーのラベルには**現在値を出す**（`▦ PJ別` `↕ 期日順`）。畳んだことで状態が見えなく
  なるのを防ぐ。**`sortKey` のように列見出しクリックでも変わる値は、取りうる値を全部
  ラベルに用意すること**（v3.114では7値。3値しか用意しないと「優先度順なのに ↕ 期日順 と
  表示される」嘘になる）。
- **検索欄を畳む場合は、検索語が入っている間は必ず開いたままにする。** 絞り込みが効いて
  いるのに入力欄が隠れていると、件数が減った理由が画面から分からなくなる。
