# CLAUDE.md — グループ計画管理アプリ 設計ドキュメント v3.91
#
最終更新：2026-08-21（v3.91）
...
## 43. タスク追加「＋」FABの共通化（v3.86）＋右下スタックの一元管理（v3.91・2026-08-21）
...
**ショートカットボタン・Toastの退避（クレーム④）**：ショートカットボタンはFAB本体の上端+クリアランスに置き、FABメニュー展開時はメニューの上端+クリアランスまで押し上げる（旧仕様と同じ考え方・数値だけ追随）。**Toastは、FABメニューの開閉に関わらず出現しうる（保存完了・エラー等はいつでも起こる）ため、開閉のたびに位置を動かすとちらつく。ショートカットボタンが最も高くなる「FABメニュー展開時」の位置を基準に、常にその上へ静的に確保する**（開閉状態を問わず重ならないことを保証するため。この設計により、Toastの通常位置はv3.90時点（84px）よりかなり高くなる（280px）が、FABメニューの3項目が積み上がった状態でもToastと重ならないことを優先した）。
...
### ErrorBarとの関係（v3.86からの記録・変更なし）

「履歴N件」バッジ単体（エラー表示が無い状態）はFABと数px〜十数pxの隙間があり、通常は重ならない。エラー行が複数（最大5件）積み上がると帯の高さが伸びFAB・展開メニューと重なりうるが、エラーは15秒で自動消去される一過性の表示のため、右下スタックの対象外と判断している（`ErrorBar.tsx`のコメントに理由を明記）。

### 機械チェック（v3.91新設）

`src/lib/layout/__tests__/bottomStack.test.ts`が、`bottomStack.ts`の定数から各要素の占有区間`[bottom, bottom+height)`を計算し、PC・モバイル×FABメニュー展開中／非展開中の4パターン全てで、どの2つの要素も重ならないことを検査する。**実装前にv3.90時点の値（PC：フッター/FAB・ショートカット/Toast、モバイル：ショートカット/Toast）に対して実際にこのロジックを走らせ、3件とも赤くなることを確認済み**（本Section冒頭の「根本原因」参照）。

### 🔴 グランドルール：右下に要素を追加するときは必ずこのスタックに載せる

画面右下（PC）／画面下端（モバイル）に新しい固定要素（バッジ・ボタン・通知等）を追加するときは、bottom値を手書きせず、必ず`src/lib/layout/bottomStack.ts`のスタックへ新しい段として追加し、`bottomStack.test.ts`の重なり検査に組み込むこと。
