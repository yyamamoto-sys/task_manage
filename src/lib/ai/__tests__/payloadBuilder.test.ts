...
describe("buildPayload — タスク拡張フィールド（start_date / parent_task_name / tags）", () => {
  it("task.start_date がペイロードに渡る", () => {
    const { payload } = buildPayload({
      projects: [makeProject({ id: "pj-1" })],
      tasks: [makeTask({ id: "t-1", project_id: "pj-1", start_date: "2026-06-01" })],
      members: [makeMember()],
      consultationType: "change",
      consultation: "x",
      scope: "all_pj",
    });
    expect(payload.projects[0].tasks[0].start_date).toBe("2026-06-01");
  });

  it("start_date が null のときは null のまま渡る", () => {
    const { payload } = buildPayload({
      projects: [makeProject({ id: "pj-1" })],
      tasks: [makeTask({ id: "t-1", project_id: "pj-1", start_date: null })],
      members: [makeMember()],
      consultationType: "change",
      consultation: "x",
      scope: "all_pj",
    });
    expect(payload.projects[0].tasks[0].start_date).toBeNull();
  });

  it("子タスクの parent_task_name に親タスク名が入る", () => {
    const { payload } = buildPayload({
      projects: [makeProject({ id: "pj-1" })],
      tasks: [
        makeTask({ id: "parent-1", name: "親タスク", project_id: "pj-1" }),
        makeTask({ id: "child-1",  name: "子タスク", project_id: "pj-1", parent_task_id: "parent-1" }),
      ],
      members: [makeMember()],
      consultationType: "change",
      consultation: "x",
    ...
    const pjTasks = payload.projects[0].tasks;
    const parent = pjTasks.find(t => t.task_name === "親タスク")!;
    const child  = pjTasks.find(t => t.task_name === "子タスク")!;
    expect(parent.parent_task_name).toBeNull();
    expect(child.parent_task_name).toBe("親タスク");
  });

  it("タグが空のとき空配列が渡る", () => {
    const { payload } = buildPayload({
      projects: [makeProject({ id: "pj-1" })],
      tasks: [makeTask({ id: "t-1", project_id: "pj-1" })],
      members: [makeMember()],
      consultationType: "change",
      consultation: "x",
      scope: "all_pj",
    });
    expect(payload.projects[0].tasks[0].tags).toEqual([]);
  });

  it("タグが設定されているときそのまま渡る", () => {
    const { payload } = buildPayload({
      projects: [makeProject({ id: "pj-1" })],
      tasks: [makeTask({ id: "t-1", project_id: "pj-1", tags: ["設計", "確認待ち"] })],
      members: [makeMember()],
      consultationType: "change",
      consultation: "x",
      scope: "all_pj",
    });
    expect(payload.projects[0].tasks[0].tags).toEqual(["設計", "確認待ち"]);
  });
});

describe("buildPayload — PJ拡張フィールド（pj_start_date）", () => {
  it("pj.start_date がペイロードの pj_start_date に渡る", () => {
    const { payload } = buildPayload({
      projects: [makeProject({ start_date: "2026-04-01" })],
      tasks: [],
      members: [makeMember()],
      consultationType: "change",
      consultation: "x",
    ...
    expect(payload.projects[0].pj_start_date).toBe("2026-04-01");
  });

  it("pj.start_date が未設定のとき null が渡る", () => {
    const { payload } = buildPayload({
      projects: [makeProject({ start_date: undefined })],
      tasks: [],
      members: [makeMember()],
      consultationType: "change",
      consultation: "x",
    ...
    expect(payload.projects[0].pj_start_date).toBeNull();
  });
});
