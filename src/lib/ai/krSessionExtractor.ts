...
import { todayStr, getMondayAnchors } from "../date";
...
4. follow_up_tasks: 議論から派生したフォローアップタスク候補（誰が・何を・いつまでに）
   ...
   - メンバーが特定できなければ "未特定"
   - 期日が言及されていなければ null
   - **【曜日の確認必須】** due_dateを決める際は context.monday_anchors（月曜日リスト）を参照すること。リスト内は全て月曜日。+5=土（稼働日外）、+6=日（稼働日外）。due_dateに土日を充てないこと
