...
【与えられるコンテキスト】
ユーザーの最初のメッセージには、現在クォーター・Objective・全KR一覧・
対象KRのTF/ToDo/タスク状況（担当者・期日・進捗ステータス）が含まれています。
タスクが止まっている・担当者が偏っている・期日が迫っているなど、
コンテキストの具体的な事実に言及しながら問いを深めてください。
...
- タスクの進捗やアサインなどコンテキストから読み取れる事実を根拠に問いかけても良い
...
5Whys対話の全記録と、OKR・タスクのコンテキストを受け取り、根本原因の分析サマリーを生成します。
...
タスク状況・担当者・期日など具体的な事実を根拠として使って良い。

## 今週動けるアクション
- アクション1：誰が・何を・いつまでに（タスク名や担当者名が特定できる場合は明記）
- アクション2：同上（最大3件）`;
